globalThis.NestedGenerator = (data) => {
    ...
    if (data instanceof Element) {
        ...
        Array.from(el.children).forEach(child => {
            let chld = NestedGenerator(child);

            if ( chld.id == undefined ) {
                childProxys.push(chld);
            } else {
                childProxys[chld.id] = chld;
            }

            // childProxys.push( NestedGenerator(child) );
        })


    } else {
        el = document.createElement(data.type || "div");
    }
